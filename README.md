...
| `data/events.json` | Home upcoming accent bar (strict next future event, or next Wednesday recurring meeting), mini-calendar & upcoming list, **`/events`** cards, **`/calendar`** month grid, **`/events/[slug]`** detail pages — `manual` (curated Meetup-style entries with `slug`, host, links, etc.) and `events` (scraped fallback). ISO `start`/`end`, optional `recurringRule`. Run **`pnpm events:validate`** after edits. |
